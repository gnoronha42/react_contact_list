...
function AddEdit({ history, match }) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    lastName: Yup.string().required("Sobrenome é obirgatorio"),
    email: Yup.string()
      .email("Email invalido")
      .required("O email é obrigatorio"),
    phone: Yup.string().required("O número é obrigatorio"),
    gender: Yup.string().required("o genero é obrigatorio"),
    password: Yup.string()
    ...
    confirmPassword: Yup.string()
      ...
      .when("password", (password, schema) => {
        if (password || isAddMode)
          return schema.required("É obrigatorio confirmar senha");
      })
  ...
  function createUser(data) {
    return userService
      .create(data)
      .then(() => {
        alertService.success("Usuario adicionado com sucesso", {
  ...
  function updateUser(id, data) {
    return userService
      .update(id, data)
      .then(() => {
        alertService.success("Usuario editado com sucesso", {
          keepAfterRouteChange: true,
        });
        history.push("..");
      })
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} onReset={reset}>
      ...
      <div className="form-grou p col-5"> 
          <label>Nome</label>
          <input
            name="firstName"
            type="text"
